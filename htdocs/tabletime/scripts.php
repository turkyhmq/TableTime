...
// Chain of if statements happen when flutter sends request to do anything regarding database
$act = '';
if(isset($_POST['act'])) $act = $_POST['act'];

$ImgPath = 'http://'.$_SERVER['HTTP_HOST'].'/tabletime/restimgs/';

// when search table via NFC
if ($act == "getTableByNFC") {
    $nfcCode = $_POST['nfc'];  // Get NFC code from POST request

    mysqli_select_db($conx, $database_conx);
    $qry = "SELECT * FROM ".$dbprefix."seattables WHERE tabnfc='".$nfcCode."'";
    $rso = mysqli_query($conx, $qry) or die(mysqli_error($conx));
    $Tables = array();
    while ($rw = mysqli_fetch_object($rso)) array_push($Tables, $rw);

    if (!empty($Tables)) {
        $rows = [$Tables];
        $Ary = array("status"=>"done", "msg"=>"Data loaded based on NFC", "rows"=>$rows);
    } else {
        $Ary = array("status"=>"error", "msg"=>"No table found with this NFC code");
        $rows = [];
    }
...
if($act == "register"):
	...
	$mobile = $_POST['mobile'];
	$email = $_POST['email'];
	// owner = 1 , employee = 2 , customer =3
    $groupkey = ["SingingCharacter.select"=>1, "SingingCharacter.owner"=>1, "SingingCharacter.user"=>3];
...
if($act == "upload"):
	...
	$Ary = array("status"=>"done", "action"=>"done", "msg"=>"image uploaded", "newimgname"=>$ImgPath.$name, "folder"=>$folder);
	$res = array("response"=>$Ary);
	echo json_encode($res, JSON_UNESCAPED_UNICODE);
endif;
...
if($act == "saveRest"):
	...
	// reload data
	mysqli_select_db($conx, $database_conx);
	$qry = "INSERT INTO ".$dbprefix."restaurants (ownerid, restname, restemail, restphone, descr, address, restpic)
	VALUES ('$userid','$name','$email','$phone','$descr','$address','$restimg')";
	$rso = mysqli_query($conx, $qry) or die(mysqli_error($conx));
...
if($act == "saveTable"):
	...
	// reload data
	mysqli_select_db($conx, $database_conx);
	$qry = "INSERT INTO ".$dbprefix."seattables (`restid`, `tabnfc`, `tabnum`, `tabchairs`, `tabstatus`, `tabposition`)
			VALUES ('$restid', '$nfc', '$tablenum', '$chairnum', '0', '$tabposition')";
	$rso = mysqli_query($conx, $qry) or die(mysqli_error($conx));
...
if($act == "getReserves"):
	...
	// reload data
	mysqli_select_db($conx, $database_conx);
	$qry = "SELECT r.*, s.*, rt.*, from_unixtime(r.reservetime, '%Y/%d/%m %H:%i:%s') as reservetimeb FROM ".$dbprefix."reservations r
			LEFT JOIN ".$dbprefix."seattables s ON s.tableid = r.tableid
			LEFT JOIN ".$dbprefix."restaurants rt ON rt.restid = s.restid
			WHERE r.userid='".$userid."'";
	if($roleid == "1")
		$qry = "SELECT r.*, s.*, rt.*, u.*, from_unixtime(r.reservetime, '%Y/%d/%m %H:%i:%s') as reservetimeb FROM ".$dbprefix."reservations r
		LEFT JOIN ".$dbprefix."seattables s ON s.tableid = r.tableid
		LEFT JOIN ".$dbprefix."restaurants rt ON rt.restid = s.restid
		LEFT JOIN ".$dbprefix."users u ON u.userid = r.userid
		WHERE rt.ownerid='".$userid."'";
	...
	$Ary = array("status"=>"done", "msg"=>"data loaded ".$qry , "rows"=>$rows);
	$res = array("response"=>$Ary);
	echo json_encode($res, JSON_UNESCAPED_UNICODE);	
endif;
?>